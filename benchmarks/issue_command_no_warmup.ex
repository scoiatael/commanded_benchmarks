...
Benchee.run(
  %{
    "without events" => fn list -> list |> Flow.from_enumerable() |> Flow.map(dispatch_no_events) |> Enum.to_list() end,
    "with events" => fn list -> list |> Flow.from_enumerable() |> Flow.map(dispatch_events) |> Enum.to_list() end
  },
  time: 40,
  warmup: 0,
  inputs: inputs,
